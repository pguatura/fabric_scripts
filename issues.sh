...
if [[ "$projectId" == "" ]];then
    ...
    text=$(echo $projects | jq -r '. | sort_by(.name) | to_entries[] | " \(.key)) (\(.value.platform)) \(.value.name) - \(.value.identifier) (\(.value.externalId))"')
    text="$text"
...
query="{\"query\":\"query TopIssues( \$externalId_0:String! , \$type_1:IssueType! , \
    \$start_2:UnixTimestamp! , \$end_3:UnixTimestamp! , \$filters_4:IssueFiltersType! , \
    \$state_5:IssueState! ) \
    {project(externalId: \$externalId_0 ) {crashlytics { \
        _issues1ctixL:issues(synthesizedBuildVersions:$versionName,eventType: \$type_1, \
        start:\$start_2,end:\$end_3,state:\$state_5,first:15,filters:\$filters_4) \
        {edges {node {externalId,createdAt,resolvedAt,title,subtitle,state,type, \
        occurrenceCount,earliestBuildVersion {buildVersion {name}}, \
        latestBuildVersion {buildVersion {name}}}}}}}}\",\"variables\": \
        {\"externalId_0\":\"$externalId\",\"type_1\":\"$type\", \
        \"start_2\":$startDate,\"end_3\":$endDate, \
        \"filters_4\":{},\"state_5\":\"$status\"}}"
